...
% 扰动强度扫描 (MHz -> Hz)
Gamma2        = 2*pi*0.01*10^(6);           % Hz
Gamma1_vals_M = linspace(0, 0.1, 21);   % MHz
Gamma1_vals   = 2*pi*Gamma1_vals_M*10^(6);
...
% 初始化数据表
AL = zeros(length(Gamma1_vals), 1);
AR = zeros(length(Gamma1_vals), 1);
...
parfor ii = 1:numel(Gamma1_vals)

    Gamma1 = Gamma1_vals(ii);   % Hz
...
subplot(2,1,1)
h1 = plot(Gamma1_vals_M, AL(:), 'r-', 'DisplayName', 'PgammaL');
hold on;
h2 = plot(Gamma1_vals_M, AR(:), 'b--', 'DisplayName', 'PgammaL');
hold off;
...
subplot(2,1,2)
h3 = plot(Gamma1_vals_M, ee(:), 'r-', 'DisplayName', 'PgammaL');
hold on;
...
saveas(gcf, '/Users/zoufen/Desktop/vEAvsGamma1.pdf')
...
% 保存到 CSV 文件
data = [xData1' yData1' yData2' yData2'];
writematrix(data, '/Users/zoufen/Desktop/vEAvsGamma1.csv');
