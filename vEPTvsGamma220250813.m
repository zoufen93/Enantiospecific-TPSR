...
Gamma1 = 2*pi*0.01; %MHz
%Gamma2 = 2*pi*0.01; %MHz
...
%beta0的范围
Gamma2_vals = linspace(0, 0.1, 21);

% 初始化数据表
PL = zeros(length(Gamma2_vals), 1);
PR = zeros(length(Gamma2_vals), 1);
ee = zeros(length(Gamma2_vals), 1);

% 开启并行
parpool('local');

parfor i = 1:length(Gamma2_vals)
    Gamma2  = 2*pi*Gamma2_vals(i);
...
subplot(2,1,1)
h1 =plot(Gamma2_vals, PL(:), 'r-', 'DisplayName', 'PgammaL');
hold on;
h2 =plot(Gamma2_vals, PR(:), 'b--', 'DisplayName', 'PgammaL');
hold off;
...
%axis([0 4 0 1]);%x轴y轴范围
title('$\Gamma_{1}/2\pi=0.01\,\mathrm{MHz}$', 'interpreter', 'latex', 'FontSize', 10);
xlabel('$\Gamma_{2}/2\pi$','interpreter', 'latex','FontSize',10);
ylabel('$\bar{P}_{\gamma}^{(L,R)}$','interpreter', 'latex','FontSize',10);
%legend('show');
grid on;

subplot(2,1,2)
h3 = plot(Gamma2_vals, ee(:), 'r-', 'DisplayName', 'PgammaL');
hold on;
...
saveas(gcf, '/Users/zoufen/Desktop/vEPTvsGamma2.pdf')
...
writematrix(data, '/Users/zoufen/Desktop/vEPTvsGamma2.csv');
