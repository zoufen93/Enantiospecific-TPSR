...
for i = 1:length(e0_vals)
    ...
    OmegaL_da = computeOmegaP(Jc,1,1,d0,dp,dm,-Es,HpL,H0L); %a-->alpha; b,d-->beta; c-->gamma
    OmegaR_da = computeOmegaP(Jc,1,1,d0,dp,dm,Es,HpR,H0R);

    OmegaL_dc = computeOmegaP(Jc,1,4,d0,dp,dm,-Es,HpL,H0L);
    OmegaR_dc = computeOmegaP(Jc,1,4,d0,dp,dm,Es,HpR,H0R);

    OmegaL_ba = computeOmegam(Jc,1,1,d0,dp,dm,-Es,HmL,H0L);
    OmegaR_ba = computeOmegam(Jc,1,1,d0,dp,dm,Es,HmR,H0R);

    OmegaL_bc = computeOmegam(Jc,1,4,d0,dp,dm,-Es,HmL,H0L);
    OmegaR_bc = computeOmegam(Jc,1,4,d0,dp,dm,Es,HmR,H0R);

    OmegaL_cb = conj(OmegaL_bc);
    OmegaL_ad = conj(OmegaL_da);
    OmegaR_cb = conj(OmegaR_bc);
    OmegaR_ad = conj(OmegaR_da);
    OmegaL = angle(-OmegaL_ad*OmegaL_dc*OmegaL_cb*OmegaL_ba);
    OmegaR = angle(-OmegaR_ad*OmegaR_dc*OmegaR_cb*OmegaR_ba);
    thetacL(i) = OmegaL;
    thetacR(i) = OmegaR;
end
...
%方法一：有限差分法（数值导数）
% 假设 e0_vals 是一维等间隔向量
deltaE = e0_vals(2) - e0_vals(1);  % 步长
%手动对 angle() 的相位进行“去缠绕”处理（unwrap）
ThetacL = unwrap(thetacL);
ThetacR = unwrap(thetacR);
dthetaL_dE = diff(ThetacL) / deltaE; % 左手性几何相位导数
dthetaR_dE = diff(ThetacR) / deltaE; % 右手性几何相位导数    
% 为了对齐维度（diff 少一个点），可以对 E0 中点取平均
e0_mid = (e0_vals(1:end-1) + e0_vals(2:end)) / 2;

% %方法二：拟合后微分（如果噪声大）
% % 拟合
% spL = spline(e0_vals, ThetacL);  % 左手性几何相位曲线
% spR = spline(e0_vals, ThetacR);  % 左手性几何相位曲线
% % 在高分辨率网格上求导
% e0_dense = linspace(min(e0_vals), max(e0_vals), 1000);
% dthetaL_dense = ppval(fnder(spL), e0_dense); % 求导后评价
% dthetaR_dense = ppval(fnder(spR), e0_dense); % 求导后评价
...
hold off;
legd=legend('$\theta_{f}^{L}$','$\theta_{f}^{R}$','location','southeast');%图例
...
ylabel('$\theta_{f}^{(L,R)}$','interpreter', 'latex','FontSize',10);%设置y轴标签
...
subplot(2,1,2)
h3 = plot(e0_mid, dthetaL_dE, '-', 'LineWidth', 1);
hold on;
h4 = plot(e0_mid, dthetaR_dE, '--', 'LineWidth', 1);
hold off;
%title('$|a\rangle=|1,0\rangle$, $|b\rangle=|1,-1\rangle$, $|c\rangle=|3,0\rangle$, $|d\rangle=|1,1\rangle$', 'interpreter', 'latex', 'FontSize', 10);
%axis([0 20 -pi pi]);%x轴y轴范围
xlabel('$E_{0}$ (kV/cm)','interpreter', 'latex','FontSize',10);%设置x轴标签
ylabel('$d[\theta_{f}^{(L,R)}]/dE_{0}$ [rad/(kV/cm)]','interpreter', 'latex','FontSize',10);%设置y轴标签
set(gca,'linewidth',0.8);%设置坐标轴线宽
%set(gca, 'xtick', [0 40 80], 'xticklabels', {'0' '40' '80'},'FontName','Times New Roman','fontsize',10)
%set(gca, 'ytick', [-1 -0.5 0 0.5 1], 'yticklabels', {'-1' '-0.5' '0' '0.5' '1'},'FontName','Times New Roman','fontsize',10)
%set(gca,'yminortick','on');%设置小刻度标签
...
%保存到 CSV 文件
data = [xData3' yData3' yData4'];
writematrix(data, '/Users/zoufen/Desktop/plot_data114.csv');

%%% Save figure
%set(gcf, 'PaperSize', [width_cm height_cm]);
%saveas(gcf, '/Users/zoufen/Desktop/dthetadE0.pdf')
