...
% dtheta的范围
dthetaf_vals = linspace(-pi/2, pi/2, 501);

% 初始化数据表
AL = zeros(length(dthetaf_vals), 1);
AR = zeros(length(dthetaf_vals), 1);
ee = zeros(length(dthetaf_vals), 1);
...
% 循环遍历计算
for i = 1:length(dthetaf_vals)
    thetacL = theta + dthetaf_vals(i);
    thetacR = -thetacL;
    ...
    % 存储数据
    %aL = imag(2*exp(-1.0j*thetacL)*rhoLgbm - 2*exp(-1.0j*theta)*rhoLgbp)/Omega2;
    %aR = imag(2*exp(-1.0j*thetacR)*rhoRgbm - 2*exp(-1.0j*theta)*rhoRgbp)/Omega2;
...
subplot(2,1,1)
h1= plot(dthetaf_vals, AL(:), 'r-');
hold on;
h2= plot(dthetaf_vals, AR(:), 'b--');
hold off;
...
xlabel('$\delta\theta$','interpreter', 'latex','FontSize',10);%设置x轴标签
...
saveas(gcf, '/Users/zoufen/Desktop/vEAvsdthetaf.pdf')
...
% 保存到 CSV 文件
data = [xData1' yData3'];
writematrix(data, '/Users/zoufen/Desktop/vEAvsdthetaf.csv');
