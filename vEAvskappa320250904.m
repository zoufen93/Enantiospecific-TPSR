...
c = 3*10^(8);%m/s
Delta1 = 0; %Hz  Delta1
Delta2 = -(2*pi*150*10^6)/sqrt(2); %Hz  Delta2
Omega1 = 2*pi*150*10^6; %Hz
Omega2 = 2*pi*0.1*10^6; %Hz
% Delta1 = 2*pi*0.1*10^6; %Hz  Delta1
% Delta2 = 2*pi*0.4*10^6; %Hz  Delta2
...
% kappa3的范围
kappa3_vals = linspace(0, 5, 101);

% 初始化数据表
AL = zeros(length(kappa3_vals), 1);
AR = zeros(length(kappa3_vals), 1);
ee = zeros(length(kappa3_vals), 1);
...
% 循环遍历计算
for i = 1:length(kappa3_vals)
    kappa3  = 2*pi*10^(6)*kappa3_vals(i);
    ...
    % 添加碰撞dephasing项
    C1_ops = {gket*gbra-bpket*bpbra, gket*gbra-bmket*bmbra, gket*gbra-aket*abra, bpket*bpbra-aket*abra,...
             bmket*bmbra-aket*abra, bmket*bmbra-bpket*bpbra};
    for cc = 1:length(C1_ops)
        C1 = C1_ops{cc};
        CL1 = kappa3/2 * (2*kron(conj(C1), C1) ...
             - kron(eye(p), C1'*C1) - kron((C1'*C1).', eye(p)));
        L0L = L0L + CL1;
        L0R = L0R + CL1;
    end
    ...
    % 存储数据
    aL  = ((omega/c)*(n*d0^(2)/ep0)*imag(2*exp(-1.0j*thetacL)*rhoLgbm - 2*exp(-1.0j*theta)*rhoLgbp))/(hbar*Omega2);
    aR  = ((omega/c)*(n*d0^(2)/ep0)*imag(2*exp(-1.0j*thetacR)*rhoRgbm - 2*exp(-1.0j*theta)*rhoRgbp))/(hbar*Omega2);
    AL(i) = aL; 
    AR(i) = aR;
    ee(i) = abs(aL-aR)/(aL+aR); 
end

delete(gcp('nocreate'));

subplot(2,1,1)
h1= plot(kappa3_vals, AL(:), 'r-');
hold on;
h2= plot(kappa3_vals, AR(:), 'b--');
hold off;
legd=legend('$L$','$R$','location','best');%图例
...
%axis([-pi/2 pi/2 0 1.1]);%x轴y轴范围
xlabel('$\kappa^{\prime}/2\pi\,\mathrm{MHz}$','interpreter', 'latex','FontSize',10);%设置x轴标签
ylabel('$A\,[m^{-1}]$','interpreter', 'latex','FontSize',10);%设置y轴标签
...
%axis([-pi/2 pi/2 0 1.1]);%x轴y轴范围
xlabel('$\kappa^{\prime}/2\pi\,\mathrm{MHz}$','interpreter', 'latex','FontSize',10);%设置x轴标签
ylabel('$v_{\mathrm{EA}}$','interpreter', 'latex','FontSize',10);%设置y轴标签
set(gca,'linewidth',0.8);%设置坐标轴线宽
%set(gca, 'xtick', [0 40 80], 'xticklabels', {'0' '40' '80'},'FontName','Times New Roman','fontsize',10)
%set(gca, 'ytick', [-1 -0.5 0 0.5 1], 'yticklabels', {'-1' '-0.5' '0' '0.5' '1'},'FontName','Times New Roman','fontsize',10)
%set(gca,'yminortick','on');%设置小刻度标签
...
%saveas(gcf, '/Users/zoufen/Desktop/vEAvskappa3.pdf')
...
% 保存到 CSV 文件
data = [xData1' yData3'];
writematrix(data, '/Users/zoufen/Desktop/vEAvskappa0.csv');
