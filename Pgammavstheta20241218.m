...
%参数
delta = 2*pi*0.5; %MHz  delta = Delta_1 + Delta_2
...
% e0的范围
theta_vals = linspace(-pi, pi, 501);

%预分配数组
PL = zeros(length(theta_vals), 1);
PR = zeros(length(theta_vals), 1);
...
for i = 1:length(theta_vals)
    ...
    odefunR = @(tR, yR) [
        ...
        -1.0j*delta*yR(4)-1.0j*0.5*Omega2*exp(1.0j*theta)*yR(2)+1.0j*0.5*Omega1*exp(1.0j*thetacR)*yR(3) % dy4/dt
    ];
...
% 绘图
subplot(2,1,1)
h1 = plot(theta_vals, PL(:), 'r-', 'DisplayName', 'P4L');
axis([-pi pi 0 0.5]);%x轴y轴范围
...
subplot(2,1,2)
h2 = plot(theta_vals, PR(:), 'b-', 'DisplayName', 'P4R');
axis([-pi pi 0 0.5]);%x轴y轴范围
xlabel('$\theta$','interpreter', 'latex','FontSize',10);
...
saveas(gcf, '/Users/zoufen/Desktop/Fig301.pdf')
...
writematrix(data, '/Users/zoufen/Desktop/plot_data_Pnvstheta.csv');
