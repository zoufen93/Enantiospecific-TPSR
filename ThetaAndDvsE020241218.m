...
for i = 1:length(e0_vals)
    ...
    OmegaL_da = computeOmegaP(Jc,3,1,d0,dp,dm,-Es,HpL,H0L); %a-->alpha; b,d-->beta; c-->gamma
    OmegaR_da = computeOmegaP(Jc,3,1,d0,dp,dm,Es,HpR,H0R);

    OmegaL_dc = computeOmegaP(Jc,3,4,d0,dp,dm,-Es,HpL,H0L);
    OmegaR_dc = computeOmegaP(Jc,3,4,d0,dp,dm,Es,HpR,H0R);

    OmegaL_ba = computeOmegam(Jc,3,1,d0,dp,dm,-Es,HmL,H0L);
    OmegaR_ba = computeOmegam(Jc,3,1,d0,dp,dm,Es,HmR,H0R);

    OmegaL_bc = computeOmegam(Jc,3,4,d0,dp,dm,-Es,HmL,H0L);
    OmegaR_bc = computeOmegam(Jc,3,4,d0,dp,dm,Es,HmR,H0R);
...
%writematrix(data, '/Users/zoufen/Desktop/plot_data132.csv');
