...
%预分配数组
PL = zeros(length(domega2_vals), 1);
PR = zeros(length(domega2_vals), 1);

for i = 1:length(domega2_vals)
    Delta2  = Delta2p - 2*pi*domega2_vals(i);
...
% 绘图
subplot(2,1,1)
h1 = plot(domega2_vals, PL(:), 'r-', 'DisplayName', 'P4L');
...
subplot(2,1,2)
h2 = plot(domega2_vals, PR(:), 'b-', 'DisplayName', 'P4R');
axis([-3 3 0 0.5]);%x轴y轴范围
xlabel('$\delta\omega_{2}/2\pi$ (MHz)','interpreter', 'latex','FontSize',10);
...
saveas(gcf, '/Users/zoufen/Desktop/Pgvsdomega2.pdf')
...
writematrix(data, '/Users/zoufen/Desktop/Pgvsdomega2.csv');
