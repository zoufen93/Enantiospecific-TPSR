...
c = 3*10^(8);%m/s
Delta1 = 2*pi*0.1*10^(6); %Hz  
Delta2p = 2*pi*0.4*10^(6); %Hz  
...
% dtheta的范围
domega2_vals = linspace(-3, 3, 201);

% 初始化数据表
AL = zeros(length(domega2_vals), 1);
AR = zeros(length(domega2_vals), 1);
...
% 循环遍历计算
for i = 1:length(domega2_vals)
    Delta2  = Delta2p - 2*pi*10^(6)*domega2_vals(i);
...
% 绘图
subplot(2,1,1)
h1 = plot(domega2_vals, AL(:), 'r-', 'DisplayName', 'AL');
axis([-3 3 0 0.5]);%x轴y轴范围
title('$\theta = \theta_{f}^{(L)}$ and $\Delta_2=\Delta_2-\delta\omega_{2}$', 'interpreter', 'latex', 'FontSize', 10);
xlabel('$\delta\omega_{2}/2\pi$ (MHz)','interpreter', 'latex','FontSize',10);
ylabel('$A^{(L)}$ ($\mathrm{m}^{-1}$)','interpreter', 'latex','FontSize',10);
%ylabel('$P_{\gamma}^{(L,max)}$','interpreter', 'latex','FontSize',10);
%legend('show');
grid on;

subplot(2,1,2)
h2 = plot(domega2_vals, AR(:), 'b-', 'DisplayName', 'AR');
%axis([-2 2 0 0.5]);%x轴y轴范围
xlabel('$\delta\omega_{2}/2\pi$ (MHz)','interpreter', 'latex','FontSize',10);
ylabel('$A^{(R)}$ ($\mathrm{m}^{-1}$)','interpreter', 'latex','FontSize',10);
...
saveas(gcf, '/Users/zoufen/Desktop/Avsdomega2.pdf')
...
writematrix(data, '/Users/zoufen/Desktop/Avsdomega2.csv');
